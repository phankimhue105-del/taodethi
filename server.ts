      ...
      sections: [
        {
          ...
          children: [
            ...
            // Answer Key Table
            ...generateDocxAnswerKey(exam.questions),
          ],
        },
      ],
    });
...
// Helper: Format Answer Key for DOCX
function generateDocxAnswerKey(questions: Question[]): Paragraph[] {
  const elements: Paragraph[] = [];
    ...
    elements.push(
      new Paragraph({
        children: [
          ...
          new TextRun({ text: `Đáp án: ${q.correctAnswer} (${q.points} điểm)`, bold: true, size: 20 }),
        ],
      })
    );
    elements.push(
      new Paragraph({
        indent: { left: 360 },
        children: [
          new TextRun({ text: 'Lời giải chi tiết: ', bold: true, italics: true, size: 18 }),
